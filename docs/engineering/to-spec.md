...
`to-spec` turns the conversation you have just had into a **[spec](https://www.aihero.dev/ai-coding-dictionary/spec)**, and publishes it to your repo's Hermes Kanban board as a single card.
...
| Decided, and the work spans several sessions | `/to-spec`, then [to-tasks](https://aihero.dev/skills-to-tasks) |
| A [wayfinder](https://aihero.dev/skills-wayfinder) map has cleared | `/to-spec #<map_issue>` |

## Prerequisites

`to-spec` publishes the spec as a board card, so [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) must have configured the Hermes Kanban board — its slug, tenant, and the triage-role → status mapping — for this repo first. The card lands at `triage`; `to-tasks` is what slices it into `ready` task cards.
...
**Why does the spec get auto-promoted to agent-ready? I don't want an agent implementing off it.**
Publishing the spec creates a board card and leaves it at `triage`; `to-tasks` (not `to-spec`) is what produces the `ready` task cards. The spec card is a decision record, not a work order. But if you run [AFK](https://www.aihero.dev/ai-coding-dictionary/afk) agents that poll for `ready` cards, the distinction isn't visible to them, and they will happily try to build the whole spec in one run instead of picking up the task slices `to-tasks` produced. This is the most-reported rough edge on the skill. Until it changes, exclude the parent spec explicitly in your AFK agent's prompt, or run `/to-tasks` to slice it into `ready` tasks before the runner reaches it.

**Why not go straight from grilling to `/to-tasks` and skip the spec?**
...
Its neighbours upstream are [grill-with-docs](https://aihero.dev/skills-grill-with-docs), which does the deciding this skill only records, and [wayfinder](https://aihero.dev/skills-wayfinder), whose finished map merges onto the chain right here. Downstream, [to-tasks](https://aihero.dev/skills-to-tasks) cuts the spec into tracer-bullet tickets for [implement](https://aihero.dev/skills-implement) to build. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
