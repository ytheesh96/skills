...
`wayfinder` takes an effort too big for one agent [session](https://www.aihero.dev/ai-coding-dictionary/session) — an idea whose **destination** you can name but whose route you cannot yet see — and charts it as a shared **map** of **decision [tickets](https://www.aihero.dev/ai-coding-dictionary/ticket)** on Hermes Kanban, then resolves them one at a time until the way is clear.

It plans, it does not do. Every ticket holds a question whose resolution is a decision, not a slice of a build to execute, and the map is finished when another [session](https://www.aihero.dev/ai-coding-dictionary/session) can implement the result without guessing. That one rule is what separates a wayfinder ticket from an ordinary implementation [task](https://www.aihero.dev/ai-coding-dictionary/task), and it is the rule agents break most often. When the map clears, wayfinder hands off; it does not carry on into code.
...
It is the heaviest, densest flow in the set, so the trigger is narrow: the effort has to be genuinely larger than one session can hold, and the route to the destination has to be foggy. The split is a clean one: `/grill-with-docs` for single-session planning, `/wayfinder` for multi-session planning.
...
| A cleared wayfinder map | [to-spec](https://aihero.dev/skills-to-spec), then [to-tasks](https://aihero.dev/skills-to-tasks) and [implement](https://aihero.dev/skills-implement) |
...
The map and its tickets live on Hermes Kanban, so wayfinder needs the board wiring that [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) lays down: it records the board slug, the tenant convention, and the notifier/subscription expectation for this repo, and wayfinder resolves the board and tenant from that per-repo config rather than a hard-coded path. Hermes Kanban is the only medium — there is no second one (no GitHub Issues, no local markdown) and no separate index document.

The tracker is not decoration. The dependency edges (`parents=[...]`) are what render the frontier on the board's own UI, and a board without native blocking degrades wayfinder to inferring blockers from the map text, which works but needs closer supervision.

## The map, the fog, and the frontier

The **map** is one tenant-scoped board plus one blocked **map closeout** [task](https://www.aihero.dev/ai-coding-dictionary/task) that holds the low-resolution context — Destination, Notes, Decisions so far, Not yet specified, Out of scope. It is an **index, not a store**: a decision lives in exactly one place, and the closeout only gists it. There is no second planning document — the closeout *is* the index. A session loads the map at low resolution and zooms into individual tickets on demand, which is what lets a map keep growing without every session paying for its whole history.
...
- **Decisions so far** — one line per closed ticket, appended as a comment on the closeout, each linking to where the detail actually lives.
...
The **frontier** is the dependency-satisfied `ready`/`running` tasks — the edge of the known. The dispatcher promotes a task to `ready` once its `parents` are all done, so the frontier fills itself as decisions land. Tasks are referred to by title throughout, never by a bare ID; a wall of task numbers is illegible in narration. A human stays in the loop through the task's assignee: when a decision is the only thing left, the worker blocks on `needs_input` and the foreground — the assignee who speaks for the user — asks, comments the answer, and unblocks.

## The four task types

Every ticket is one of four types, and is either **[HITL](https://www.aihero.dev/ai-coding-dictionary/human-in-the-loop)** — worked with a human who speaks for themselves — or **[AFK](https://www.aihero.dev/ai-coding-dictionary/afk)**, driven by the agent alone. A HITL ticket only resolves through the live exchange; an agent that answers its own [grilling](https://www.aihero.dev/ai-coding-dictionary/grilling) questions has broken it.

| Type | Mode | Reach for it when | Resolved by |
| --- | --- | --- | --- |
| `research` | AFK | A fact outside the working directory is blocking a decision. | The [research](https://aihero.dev/skills-research) workflow, run as a worker session — independent research tasks may run in parallel |
| `prototype` | AFK then HITL | "How should this look" or "how should this behave" — a question talking cannot settle. | [prototype](https://aihero.dev/skills-prototype), with the built artifact linked from the ticket as an asset |
| `decision` / `grilling` | HITL | The default. The question can be settled by talking it through. | [grilling](https://aihero.dev/skills-grilling) plus [domain-modeling](https://aihero.dev/skills-domain-modeling), in a fresh session |
...
One worker [session](https://www.aihero.dev/ai-coding-dictionary/session) resolves one ticket. Independent `research` tickets may run in parallel; `parents` control the rest. When a human decision is the only thing left on a ticket, the worker calls `kanban_block(kind="needs_input")`, the foreground asks, comments the answer, and unblocks — the human stays in the loop through the assignee, not through a second document.
...
- You can look at your board and see which tickets are takeable without opening the map — that is the frontier rendering itself through native blocking.
- A session resolves one ticket, posts the answer as a resolution comment, closes it, and leaves one line on the closeout's *Decisions so far*. Then it stops.
...
- When the opening breadth-first chart turns up no fog at all, the skill stops and tells you the effort is small enough to skip the map.
