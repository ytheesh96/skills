...
- **A huge, foggy effort — a greenfield project or a huge feature build, too big for one session** → **`/wayfinder`**, the most cognitively demanding flow here. When the way from here to the destination isn't visible yet, you raise it as a **map task**: the board's orchestrator charts the fog into parent tasks of four kinds — `[RESEARCH]`, `[PROTOTYPE]`, `[DECISION]`, `[OPERATION]` — and resolves them one charting run at a time, pushing the fog back until the way is clear. The task graph *is* the map; there is no separate index to maintain. Where **`/grill-with-docs`** sharpens an idea you can hold in one session, wayfinder is for the idea you can't — and it's slower and denser, so save it for exactly that, never a well-scoped feature.

  When the fog clears, **it hands off, it doesn't build**: the map fans out `to-spec` and `to-tasks` as child tasks — `to-spec` collapses the resolved threads into a buildable plan, then the tickets feed `/implement` as usual. Looping the map straight into `/implement` skips that collapse and throws the linked detail away — go straight to `/implement` only when the effort turned out genuinely small.
