...
3. **Branch — is this a multi-session build?**
   - **Yes** → **`/to-spec`** (turn the thread into a spec), then **`/to-tasks`** to split it into tracer-bullet tasks, each declaring its **blocking edges**. `/to-tasks` publishes the tasks to the repo's Hermes Kanban board — the board slug and tenant come from the per-repo config doc that the setup skill writes — and the blocking edges become `parents`, so any task whose blockers are done is grabbable. Kick off **`/implement`** per task, **`/clear`ing context between each one**. Each task is self-contained, so the last one's context is disposable.
